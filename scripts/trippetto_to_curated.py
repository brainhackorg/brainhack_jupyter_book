...
def main():
    ...
    raw = pd.read_csv(
        f"{DATA_DIR}/affiliation_and_consent_for_the_brainhack_neuroview_preprint_raw.tsv",
        sep="\t",
        header=[0, 1, 2],
    )
    tpt = pd.read_csv(
        f"{DATA_DIR}/affiliation_consent_and_contributions_for_the_brainhack_consortium_tripetto_raw.csv"
    )

    if emails:
        source_osf = pd.read_csv(
            f"{DATA_DIR}/affiliation_and_consent_for_the_brainhack_neuroview_preprint_source.tsv",
            sep="\t",
            header=[0, 1, 2],
        )
        source_tpt = pd.read_csv(
            f"{DATA_DIR}/affiliation_consent_and_contributions_for_the_brainhack_consortium_tripetto_source.csv"
        )
        ...
        raw = pd.concat(
            [raw, source_osf[source_osf.columns[14]]], axis=1, join="inner"
        ).reset_index()

        tpt = tpt.set_index("First and Last Name")
        source_tpt = source_tpt.set_index("First and Last Name")
        tpt = pd.concat(
            [tpt, source_tpt["Email address"]], axis=1, join="inner"
        ).reset_index()
    ...
    for i, row in tpt.iterrows():
        ...
        if len(row["First and Last Name"]) == 2:
            ...
            raw.loc[raw_ix, [raw.columns[9], raw.columns[10]]] = row[
                "First and Last Name"
            ]
        else:
            # curated.loc[curate_ix, ["First", "Middle","Last"]] = row["First and Last Name"]
            raw.loc[raw_ix, [raw.columns[9], raw.columns[11], raw.columns[10]]] = row[
                "First and Last Name"
            ]
...
if __name__ == "__main__":
    ...
    print("Not needed since the paper is now published")
